<template>
  <p-layout>
    <!-- 顶部 -->
    <el-input placeholder="请输入公司名称" v-model="gjzString" style="width: 300px;margin-left: 20px">
    ...
    <!-- 新增 -->
    <addWeb v-if="addVisible"
            :visible="addVisible"
            @close="addVisible = false"
            @ok="addOk"/>

    <!-- 修改 -->
    <editWeb v-if="editVisible"
             :visible="editVisible"
             :obj="currentObj"
...
import editWeb from "./components/editWeb.vue";
import webInfo from "./components/webInfo.vue";
import {deleteWeb , getWebList} from "@/api/website";
import commonLabel from "@/components/commonLabel"

export default {
	name: 'website',
  components: { webItem , addWeb , editWeb , webInfo , commonLabel },
	data() {
		return {
      gjzString: '',       //搜索关键字
  ...
  methods: {
    /*搜索*/
    queryList(){
      getWebList({websiteName: this.gjzString}).then(res=>{
    ...
    /*删除*/
    delLabel(item){
      this.$modal.confirm('是否确认删除选中的网址['+ item.labelName+']？').then(function() {
        return  deleteWeb(item.id);
      }.bind(this)).then(() => {
